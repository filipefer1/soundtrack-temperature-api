...
class Temperature {
  ...
  async fetchTemperatureByCityName(cityName) {
    try {
      const response = await this.request.get(`${_config.temperature.TEMPERATURE_API_URL}?q=${cityName}&units=${this.temperatureUnit}&appid=${_config.temperature.TEMPERATURE_API_TOKEN}`);
  ...
  async fetchTemperatureByCoords(lat, lon) {
    try {
      const response = await this.request.get(`${_config.temperature.TEMPERATURE_API_URL}?lat=${lat}&lon=${lon}&units=${this.temperatureUnit}&appid=${_config.temperature.TEMPERATURE_API_TOKEN}`);
      return this.normalizeResponse(response.data);
    } catch (err) {
      if (HTTP.Request.isRequestError(err)) {
        throw new TemperatureResponseError(`Error: ${JSON.stringify(err.response.data)} Code: ${err.response.status}`);
